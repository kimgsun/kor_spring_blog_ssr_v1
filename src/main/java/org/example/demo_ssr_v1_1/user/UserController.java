...
public class UserController {
    ...
    public String updateForm(Model model, HttpSession session) {
        // 1. 인증 검사 (O) - 로그인 안 한 사람한텐 보여줄 수 없음
        // 2. 유효성 검사 (X)
        // 인증 검사를 하려면 세션 메모리에 접근해서 사용자의 정보가 있는지 없는지 여부 확인
        // A 사용자가 요청 시 --> 웹 서버 --> 톰캣(WAS)
        //                                HttpServletRequest 객체와 Response 객체를 만들어서 스프링 컨테이너에게 전달해줌
    ...
    public String loginProc(UserRequest.LoginDTO loginDTO, HttpSession session) {
        ...
        // 4. 로그인 성공 또는 실패 처리
        // 5. 웹 서버는 바보라서 사용자의 정보를 세션 메모리에 저장 시켜야 다음 요청이 오더라도 알 수 있어 - 세션 저장 처리
